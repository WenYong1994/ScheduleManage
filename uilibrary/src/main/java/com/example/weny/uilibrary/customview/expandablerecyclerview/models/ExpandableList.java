package com.example.weny.uilibrary.customview.expandablerecyclerview.models;
...
public class ExpandableList {
  ...
  private int numberOfVisibleItemsInGroup(int group) {
    ...
    if (groups.get(group).isExpand()) {
      return groups.get(group).getItemCount() + 1+groups.get(group).getLastVisibleCount();
    } else {
      return 1+groups.get(group).getLastVisibleCount();
    }
  }
  ...
  public ExpandableListPosition getUnflattenedPosition(int flPos) {
    ...
    int adapted = flPos;
    for (int i = 0; i < groups.size(); i++) {
      ExpandableGroup group = groups.get(i);
      ...
      if (adapted == 0) {
        ...
        if(group.isExpand()){//展开的 这样返回没有问题
          return ExpandableListPosition.obtain(ExpandableListPosition.CHILD, i, adapted - 1, flPos);
        }else {
          int childPos = group.getItemCount()+adapted-1;
          if(childPos<group.getItemCount()+group.getLastVisibleCount()){
            return ExpandableListPosition.obtain(ExpandableListPosition.CHILD, i, childPos, flPos);
          }
        }
      }
      adapted -= groupItemCount;
    }
    return ExpandableListPosition.obtain(ExpandableListPosition.UNKNOW ,0, 0, flPos);
  }
